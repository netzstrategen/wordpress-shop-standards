...
class WooCommerce {
   ...
   *
   * When a SKU or Custom ID number (possibly of a product variation) is used
  ...
  public static function woocommerce_product_after_variable_attributes($loop, $variation_id, $variation) {
    ...
    echo '<div style="clear:both">';
    woocommerce_wp_text_input([
      'id' => '_custom_variable_custom_erp_id[' . $loop . ']',
      'label' => __('ERP/Inventory ID', Plugin::L10N),
  ...
  public static function woocommerce_save_product_variation($post_id) {
    if (isset($_POST['variable_sku'], $_POST['variable_post_id']) && (isset($_POST['_custom_variable_gtin']) || isset($_POST['_custom_variable_ID']))) {
      $variable_sku = $_POST['variable_sku'];
      $variable_post_id = $_POST['variable_post_id'];
      foreach ($variable_post_id as $variation_id) {
        $gtin = $_POST['_custom_variable_gtin'] ?: '';
        $custom_ID = $_POST['_custom_variable_ID'] ?: '';
        foreach ($variable_sku as $j => $sku) {
          ...
          // ERP/Inventory ID field.
          if (isset($custom_ID[$j])) {
            update_post_meta($variation_id, '_custom_erp_id', $custom_ID[$j]);
          }
